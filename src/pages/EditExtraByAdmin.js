...
export default function UbahKelas() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/api/extra/ ` + param.id_extra)
      .then((response) => {
  ...
  const submitActionHandler = async (event) => {
    try {
      ...
      await axios.put(`http://localhost:8080/api/extra/` + param.id_extra, {
        namaExtra: namaExtra,
        status: status,
      });
      ...
      window.location.href = "/extra/" + param.id_sekolah;
    } catch (error) {
