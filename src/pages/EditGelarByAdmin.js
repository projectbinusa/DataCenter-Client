...
export default function EditGelar() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/api/gelarPendidikan/${param.id_gelar}`)
      .then((response) => {
  ...
  const submitActionHandler = async (event) => {
    ...
    await axios
      .put(`http://localhost:8080/api/gelarPendidikan/${param.id_gelar}`, {
        namaGelar: namaGelar,
      ...
      .then(() => {
        ...
        window.location.href = "/gelar/" + param.id_sekolah;
      })
      .catch((error) => {
