...
export default function DataKelas() {
  ...
  const de = async (id) => {
    await Swal.fire({
      title: "Anda yakin?",
      text: "Yakin ingin menghapus data gelar ini?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        axios.delete("http://localhost:8080/api/gelarPendidikan/" + id).then(() => {
          Swal.fire({
            position: "center",
            icon: "success",
    ...
    if (isChecked.length !== 0) {
      await Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .delete(
              `http://localhost:8080/api/gelarPendidikan?ids=` + isChecked.toString()
            )
            .then(() => {
              Swal.fire({
                position: "center",
        ...
        <div className="flex justify-center w-[100%]">
          <main className="s-content w-[390px] md:w-[1125px] px-5 md:px-10 py-5">
            ...
            <div className="grid justify-center mb-5">
                <h3 className="text-4xl  "> Data Gelar  </h3>
              </div>
